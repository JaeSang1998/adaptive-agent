...
[src/adaptive_agent/agent/](src/adaptive_agent/agent/) — [core/](src/adaptive_agent/agent/core/) · [planner.py](src/adaptive_agent/agent/planner.py) · [session.py](src/adaptive_agent/agent/session.py) · [compaction.py](src/adaptive_agent/agent/compaction.py) · [events.py](src/adaptive_agent/agent/events.py)
...
- **프레임워크 없이 순수 tool loop** — LangGraph/LangChain 배제. 모든 상태 전이가 [agent/core/](src/adaptive_agent/agent/core/) 패키지의 `AgentCore.__init__.py` 한 파일에서 추적 가능 (codegen/repair/meta handler 는 책임 단위 분할).
- **`max_steps=15`** — 무한루프 방지. step 한계를 초과하면 정리 메시지로 종료.
...
```
AgentCore (agent/core/__init__.py — 메인 루프 + dispatch + 공통 콜백)
  ├─ Planner            → LLM → PlannerDecision (tool_call | text)
  ├─ Session            → messages, temp_tools, plan, recent_actions, observations
  ├─ Registry           → builtin / persistent / session 도구 lookup
  │
  ├─ CodeGenHandler     (core/codegen.py) — generate_code 파이프라인
  ├─ RepairHandler      (core/repair.py)  — repair_tool 루프
  ├─ MetaHandlers       (core/meta.py)    — think / ask_user / update_plan / 일반 도구 dispatch
  │
  └─ 자유 함수: refs.resolve_refs ($ref dehydration) · workspace.workspace_context (planner grounding)
```
</details>
...
| `run_bash` | 셸 명령 실행 | ✓ (세션 캐시: normal 명령은 'a=항상허용' 가능, warn 이상은 매번 fresh approval) |
...
ToolsLayer
  ...
  │     └─ 5. 구조 검사 (class 정의 차단 + 위험한 dunder set 차단: __class__/__bases__/__subclasses__/__mro__/__globals__/__builtins__/__import__/__getattribute__/__reduce__/__reduce_ex__/__dict__)
  │
  ├─ Runner (subprocess)
...
[src/adaptive_agent/llm/prompts.py](src/adaptive_agent/llm/prompts.py) (단일 파일, ~390 lines) — LLM Layer 내부 모듈이지만, 프로젝트에서 **가장 많은 시행착오를 거친 부분**이므로 별도 섹션으로 분리합니다.
...
```
main.py → agent/core/__init__.py (AgentCore)
             ├→ agent/core/{codegen,repair,meta,refs,workspace}.py
